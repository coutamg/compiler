...
public class SimpleParser {
    ...
    private SimpleASTNode assignmentStatement(TokenReader tokens) throws Exception {
        ...
        if (token != null && token.getType() == TokenType.Identifier) { // 这里读取 age(1)
            token = tokens.read();      //读入标识符
            node = new SimpleASTNode(ASTNodeType.AssignmentStmt, token.getText());
            token = tokens.peek();      //预读，看看下面是不是等号
            if (token != null && token.getType() == TokenType.Assignment) { // 读取 =
                tokens.read();          //取出等号
                SimpleASTNode child = additive(tokens); // 读取 = 后面的 additiveExpression
                if (child == null) {    //出错，等号右面没有一个合法的表达式
                    ...
                    token = tokens.peek();  //预读，看看后面是不是分号 最后读取 ;
                    if (token != null && token.getType() == TokenType.SemiColon) {
                        tokens.read();      //消耗掉这个分号
    ...
    private SimpleASTNode intDeclare(TokenReader tokens) throws Exception {
        SimpleASTNode node = null;
        Token token = tokens.peek();
        if (token != null && token.getType() == TokenType.Int) { // 这里取出 'int'
            token = tokens.read();
            if (tokens.peek().getType() == TokenType.Identifier) { // 这里取出 Identifier
                token = tokens.read();
